...
# All of the above are examples of so called `map` operations:
...
# The last two are examples of `filter` operations:
# Creating a list with only selected elements from an original list.

# List comprehensions can easily describe a mix of map and filer operations:
[ 3*x for x in lst if x%2==0 ]
    # Creates a list of triples of the even numbers in lst
...
# Oops, but we want only pairs of DISTINCT teams:
...
[ (a,b,c) for a in range(1,20)
          ...
          for c in range(1,35)
          if a**2+b**2==c**2 ]
# But (3,4,5) and (4,3,5) are "duplicates" in some sense...
# We could eliminate them with another condition,
...
[ d[w] for w in t.split() if w in d ]
[ d[w] if w in d else w  for w in t.split() ]
l2 = [ d[w] if w in d else w for w in t.split() ]
" ".join(l2)
# Another way to do it:
l2 = [ d.get(w, w) for w in t.split() ]
l2
